...
%% All
%% -----------------
%% Psi_All: All eigenfunction
%% E_All: All eigenvalues (NOT taking account for degeneracy)
%% E_Array: All eigenvalues with degeneracy
function [Psi_All E_All E_Complete] = Diag_All(z, N, param)
  Psi_All = cell(1,N+1);
  E_All = cell(1,N+1);
  ...
  for j=1:n_cell
    if j==1
      E_Complete = [E_Complete; E_All{j}];
    elseif j==2
      E_Complete = [E_Complete; repmat(E_All{j}, z-1, 1)];
    else
      n_rep = (z-2)*z*(z-1)^(j-3);
      E_Complete = [E_Complete; repmat(E_All{j}, n_rep, 1)];
    endif
  endfor
  E_Complete = sort(E_Complete);
endfunction
